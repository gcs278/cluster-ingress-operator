...
// TestLoadBalancerServiceChangedSubnetsChangeWithLBType verifies that loadBalancerServiceChanged
// returns the updated the subnet annotation ONLY when the load balancer type changes.
func TestLoadBalancerServiceChangedSubnetsChangeWithLBType(t *testing.T) {
	testCases := []struct {
		name                       string
		current, desired, expected *corev1.Service
		expectedChange             bool
	}{
		{
			name: "subnets do not change when lb type is empty",
			current: &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"service.beta.kubernetes.io/aws-load-balancer-subnets": "subnet-foo",
					},
				},
			},
			desired: &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"service.beta.kubernetes.io/aws-load-balancer-subnets": "subnet-bar",
					},
				},
			},
			expected:       nil,
		...
		{
			name: "subnets do not change when lb type is the same",
			current: &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
		...
		{
			name: "subnets change when lb type is different",
			current: &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						"service.beta.kubernetes.io/aws-load-balancer-subnets": "subnet-foo",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			changed, updated := loadBalancerServiceChanged(tc.current, tc.desired)
			assert.Equal(t, tc.expected, updated)
			assert.Equal(t, tc.expectedChange, changed)
		})
	}
}
