...
func (r *reconciler) ensureNodePortService(ic *operatorv1.IngressController, deploymentRef metav1.OwnerReference) (bool, *corev1.Service, error) {
	...
	case !wantService && haveService:
		if !ownLBS {
			return false, nil, fmt.Errorf("refusing to delete nodeport service not owned by ingress controller: %s", controller.LoadBalancerServiceName(ic))
		}
	...
	case wantService && haveService:
		if !ownLBS {
			return false, nil, fmt.Errorf("refusing to update nodeport service not owned by ingress controller: %s", controller.LoadBalancerServiceName(ic))
		}
